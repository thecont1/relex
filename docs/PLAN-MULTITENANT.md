...
- [x] **1.1 Move tenant assets into tenant folders.**
...
- [x] **1.2 Layered `publicDir` in `vite.config.ts`.** Point Vite's `publicDir` at `tenants/{slug}/public` when it exists, falling back to the root `public/`. If shared assets are needed, copy root `public/` into the build via a small plugin or keep shared assets in the tenant dir.
...
- [x] **1.3 Update docs.** Adjust the paths in `TENANT_ONBOARDING.md` (steps 3–4) to the new tenant-scoped locations.
...
- [x] **2.1 Generate a synthetic test workbook.** Write a small script
...
- [x] **2.2 Add a placeholder logo** at
  `tenants/test-dept/public/assets/test-dept/logo.png` (simple generated
  wordmark is fine).
- [x] **2.3 Verify end to end.** `bun run dev:test-dept` loads with test-dept
  branding and data; `bun run build:all` succeeds for both tenants.
- [x] **2.4 Header background check.** The desktop header background is
...
- [x] **3.1 Drive the accent color from config on desktop.** The desktop
...
- [x] **3.2 Clean tenant-specific comments** in `src/styles/app.css`
  (lines ~903, ~954, ~1062 reference CeNSE/cense.iisc.ac.in). Reword to
  generic descriptions.
- [x] **3.3 Final sweep.** `rg -i "cense|iisc|nanoscience" src/` should return
...
- [x] **4.1 Replace the name heuristic with explicit role mapping.** Add an
...
- [x] **4.2 Validate the tenant config at build time.** In `vite.config.ts`
  ...
  malformed config.
- [x] **4.3 Document the limitation** in `TENANT_ONBOARDING.md`: tenants must
...
- [x] **5.1 Dynamic `build:all`.** The script hardcodes `cense` and
...
- [x] **5.2 Core-version consistency check.** In the build, compare
...
- [x] **5.3 Changelog entry.** Add the multi-tenant restructure to
...
- [x] **7.1** `rg -i "cense|iisc" src/` → no tenant-specific code/branding in
  core (only generic schema semantics remain).
- [x] **7.2** Fresh-clone simulation: `bun install && bun run build:all` → both
  tenants build; test-dept required zero core changes.
- [x] **7.3** CeNSE regression check: `bun run dev:cense`, verify layout,
  hover, focus/pin, accessible view, globe view, exports, and mobile view
  against the current production behavior (visual pass at
  http://localhost:5173).
- [x] **7.4** `bun run typecheck` clean.
